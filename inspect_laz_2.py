...
# classification values along with the number of points in each class.
# It accepts either a single file or a folder containing multiple LAS/LAZ files.
...
def analyze_las_classifications(las_file_path):
    ...
    along with the number of points in each class.
    ...
    try:
        ...
        with laspy.open(las_file_path, mode="r") as las_file:
            ...
            # Get unique classification values and their counts
            unique_classifications, counts = np.unique(
                las_data.classification, return_counts=True
            )

            # Print the results
            if unique_classifications.size > 0:
                print("Classification values and point counts:")
                print("---------------------------------------------")
                ...
                # Iterate through the unique values, counts, and their descriptions
                for cls_value, count in zip(unique_classifications, counts):
                    # Use the hardcoded dictionary to get the description
                    cls_description = CLASSIFICATION_LOOKUP.get(
                        int(cls_value), "Unknown"
                    )
                    pct = 100.0 * count / num_points if num_points > 0 else 0
                    print(
                        f"  Value: {int(cls_value):<3} -> {count:>{count_width},} points "
                        f"({pct:5.2f}%) -> {cls_description}"
                    )
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyzes LAS/LAZ file(s) to find and print all unique classification "
                    "values with point counts per class. Accepts either a single file or "
